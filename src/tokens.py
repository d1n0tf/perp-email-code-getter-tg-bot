...
def create_tokens_routes(
    ...
    *,
    ...
    async def page(request: Request) -> HTMLResponse:
        ...
        error = ""
        if key is not None and key.is_exhausted:
            error = exhausted_message(key)
        return user_response(key=key, error=error)

    async def activate(request: Request) -> HTMLResponse:
        form = await read_form(request)
        if not valid_csrf(request, form, TOKENS_USER_CSRF_COOKIE):
            return RedirectResponse(url="/ai/tokens", status_code=303)
        ...
        if key is None:
            return user_response(
                error="\u041a\u043b\u044e\u0447 \u0434\u043e\u0441\u0442\u0443\u043f\u0430 \u043d\u0435 \u0441\u0443\u0449\u0435\u0441\u0442\u0432\u0443\u0435\u0442.",
                submitted_code=access_code,
                status_code=404,
            )
        if key.is_exhausted:
            response = user_response(key=key, error=exhausted_message(key), submitted_code=access_code)
        else:
            response = user_response(
                key=key,
                notice="\u041a\u043b\u044e\u0447 \u0443\u0441\u043f\u0435\u0448\u043d\u043e \u0430\u043a\u0442\u0438\u0432\u0438\u0440\u043e\u0432\u0430\u043d. API-\u043a\u043b\u044e\u0447 \u0433\u043e\u0442\u043e\u0432 \u043a \u0438\u0441\u043f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u043d\u0438\u044e.",
                submitted_code=access_code,
            )
        response.set_cookie(
            TOKENS_ACCESS_COOKIE,
            access_code,
            httponly=True,
            samesite="lax",
            secure=True,
...
def exhausted_message(key: TokenKey) -> str:
    return (
        "\u0422\u043e\u043a\u0435\u043d\u044b \u0431\u044b\u043b\u0438 \u043f\u043e\u043b\u043d\u043e\u0441\u0442\u044c\u044e \u0438\u0441\u043f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u043d\u044b. "
        f"\u0414\u0430\u0442\u0430: {format_datetime(key.exhausted_at or key.activated_at)}"
    )
...
def render_tokens_page(
    *,
    ...
    info = render_key_information(key) if key is not None else ""
    instructions = render_instructions(key) if key is not None and not key.is_exhausted else ""
    content = f"""
    <main class='page'>
      <section class='card intro'>
        <h1>\u0421\u0435\u0440\u0432\u0438\u0441 \u0430\u043a\u0442\u0438\u0432\u0430\u0446\u0438\u0438</h1>
        <p>\u0417\u0434\u0435\u0441\u044c \u0432\u044b \u0441\u043c\u043e\u0436\u0435\u0442\u0435 \u0430\u043a\u0442\u0438\u0432\u0438\u0440\u043e\u0432\u0430\u0442\u044c \u0438 \u0438\u0441\u043f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u0442\u044c API \u043a\u043b\u044e\u0447\u0438 \u0441 \u0442\u043e\u043a\u0435\u043d\u0430\u043c\u0438 \u0434\u043b\u044f \u0441\u0435\u0440\u0432\u0438\u0441\u043e\u0432 Claude / Codex / Grok / Google \u0438 \u0434\u0440\u0443\u0433\u0438\u0435.<br>\u0414\u043b\u044f \u044d\u0442\u043e\u0433\u043e \u0441\u043b\u0435\u0434\u0443\u0439\u0442\u0435 \u0438\u043d\u0441\u0442\u0440\u0443\u043a\u0446\u0438\u0438 \u043d\u0438\u0436\u0435.</p>
      </section>
      <section class='card'>
        ...
        <form method='post' action='/ai/tokens' class='activation-form'>
          ...
          <label for='access_code'>\u041a\u043b\u044e\u0447 \u0434\u043e\u0441\u0442\u0443\u043f\u0430:</label>
          ...
          <p class='hint'>\u2794 \u0417\u0434\u0435\u0441\u044c \u0432\u0432\u043e\u0434\u0438\u0442\u0435 \u043a\u043b\u044e\u0447 \u0434\u043e\u0441\u0442\u0443\u043f\u0430, \u043a\u043e\u0442\u043e\u0440\u044b\u0439 \u043f\u043e\u043b\u0443\u0447\u0438\u043b\u0438 \u043e\u0442 \u043f\u0440\u043e\u0434\u0430\u0432\u0446\u0430.</p>
          <button class='primary wide' type='submit'>\u0410\u041a\u0422\u0418\u0412\u0418\u0420\u041e\u0412\u0410\u0422\u042c \u041a\u041b\u042e\u0427</button>
        </form>
      </section>
      {info}
      {instructions}
    ...
    """
    return HTMLResponse(render_layout("\u0421\u0435\u0440\u0432\u0438\u0441 \u0430\u043a\u0442\u0438\u0432\u0430\u0446\u0438\u0438 API \u043a\u043b\u044e\u0447\u0435\u0439", content))


def render_key_information(key: TokenKey) -> str:
    status = (
    ...
    <section class='card info-card'>
      <h2>\u0418\u041d\u0424\u041e\u0420\u041c\u0410\u0426\u0418\u042f</h2>
      <dl class='details'>
        <dt>\u041f\u043e\u0434\u043a\u043b\u044e\u0447\u0435\u043d\u043d\u044b\u0439 \u0441\u0435\u0440\u0432\u0438\u0441:</dt><dd>{html.escape(key.service.upper())}</dd>
        <dt>\u0414\u0430\u0442\u0430 \u0430\u043a\u0442\u0438\u0432\u0430\u0446\u0438\u0438 \u043a\u043b\u044e\u0447\u0430:</dt><dd>{format_datetime(key.activated_at)}</dd>
        <dt>\u041a\u043e\u043b\u0438\u0447\u0435\u0441\u0442\u0432\u043e \u0442\u043e\u043a\u0435\u043d\u043e\u0432:</dt><dd>{format_tokens(key.token_limit)}</dd>
        <dt>\u041e\u0441\u0442\u0430\u0432\u0448\u0438\u0435\u0441\u044f \u0442\u043e\u043a\u0435\u043d\u044b:</dt><dd>{format_tokens(key.remaining_tokens)} \u0438\u0437 {format_tokens(key.token_limit)}</dd>
        <dt>\u0421\u0442\u0430\u0442\u0443\u0441:</dt><dd>{status}</dd>
        <dt>API \u043a\u043b\u044e\u0447:</dt><dd><code class='api-key'>{html.escape(key.api_key)}</code></dd>
      </dl>
    </section>
    """
...
def instruction_command(application: str, system: str, api_key: str) -> str:
    ...
    if application == "Cheap Code":
        return f"npm install -g @cheapcode/cli@latest && echo 'Установите API ключ в настройках приложения: {api_key}'"
    if system == "Windows":
        return f"$env:CVC_API_KEY='{api_key}'; iex(irm '{url}')"
    return f"bash <(curl -fsSL '{url}') '{api_key}'"


def render_instructions(key: TokenKey) -> str:
    cards: list[str] = []
    for _, _, applications in INSTRUCTION_GROUPS:
        for application in applications:
            for system in INSTRUCTION_SYSTEMS_BY_APP[application]:
                ...
                command = html.escape(instruction_command(application, system, key.api_key))
                shell = "PowerShell" if system == "Windows" else "Терминал"
                description = "Ручная настройка с готовым скриптом"
                cards.append(f"""
                <article class='instruction-card' id='instruction-card-{slug}' data-provider-app='{html.escape(application)}' data-instruction-system='{system}' hidden>
                  ...
                  <p><strong>• Выбрано:</strong> {html.escape(application)}<br><strong>• Описание:</strong> {description}<br><strong>• Операционная система:</strong> {html.escape(system)}</p>
                  <ol>
                    <li>Откройте {shell} на машине, где запускается выбранное приложение.</li>
                    <li>Скопируйте и выполните скрипт ниже. API ключ уже подставлен автоматически.</li>
                    <li>Перезапустите приложение после завершения настройки.</li>
                  </ol>
                  <p class='warning'>⚠️ Копируйте скрипт полностью или воспользуйтесь кнопкой «Скопировать».</p>
                  <pre id='instruction-script-{slug}'>{command}</pre>
                  <button type='button' class='primary copy-instruction' data-copy-target='instruction-script-{slug}'>Скопировать</button>
                  <p class='hint'>Для удаления настройки повторите инструкцию из документации сервиса или удалите добавленные строки из конфигурации.</p>
                </article>""")
    groups = "".join(
        f"<button type='button' class='choice {'active' if index == 1 else ''}' data-instruction-provider='{key}'>{html.escape(label)}</button>"
    ...
    <section class='card instructions'>
      ...
      <p>Если вы не знаете как использовать API ключ, мы поможем, для начала выберите через что вы будете использовать API</p>
      <h3 class='step-title'>1. Выберите сервис</h3>
      <div class='choice-row' role='group' aria-label='Сервис'>{groups}</div>
      <h3 class='step-title'>2. Выберите приложение</h3>
      <div class='choice-row instruction-apps' role='group' aria-label='Приложение'>{apps}</div>
      <h3 class='step-title'>3. Выберите операционную систему</h3>
      <div class='choice-row' role='group' aria-label='Операционная система'>{systems}</div>
      <div class='selected-line'>• Выбрано: <strong id='selected-method'>Claude Code CLI</strong><br>• Описание: <strong id='selected-description'>Ручная CLI-настройка</strong><br>• Операционная система: <strong id='selected-os'>Windows</strong></div>
    ...
    *,
    ...
    <main class='page admin-page'>
      ...
      <section class='card'>
        ...
        <div class='table-wrap'><table><thead><tr><th>ID</th><th>\u0414\u0430\u0442\u0430 \u0441\u043e\u0437\u0434\u0430\u043d\u0438\u044f</th><th>\u041a\u041b\u042e\u0427</th><th>API KEY</th><th>\u0422\u041e\u041a\u0415\u041d\u041e\u0412</th><th>\u0418\u0421\u041f\u041e\u041b\u042c\u0417\u041e\u0412\u0410\u041d\u041e</th><th>\u0421\u0422\u0410\u0422\u0423\u0421</th><th>\u0423\u041f\u0420\u0410\u0412\u041b\u0415\u041d\u0418\u0415</th></tr></thead>
        <tbody>{rows or "<tr><td colspan='8' class='empty'>\u041a\u043b\u044e\u0447\u0435\u0439 \u043f\u043e\u043a\u0430 \u043d\u0435\u0442.</td></tr>"}</tbody></table></div>
        {forms}
      </section>
    </main>"""
    return HTMLResponse(render_layout("\u0410\u0434\u043c\u0438\u043d-\u043f\u0430\u043d\u0435\u043b\u044c \u043a\u043b\u044e\u0447\u0435\u0439", content))
...
def render_admin_rows(keys: list[TokenKey], csrf_token: str) -> tuple[str, str]:
    ...
    for key in keys:
        ...
        <td><code class='api-preview'>{html.escape(key.api_key)}</code></td><td>{format_tokens(key.token_limit)}</td>
...
(function() {{
  ...
  document.querySelectorAll('.copy-instruction').forEach(function(button) {{ button.addEventListener('click',function() {{ var target=document.getElementById(button.dataset.copyTarget); if(!target) return; navigator.clipboard.writeText(target.textContent).then(function() {{ var old=button.textContent; button.textContent='Скопировано'; setTimeout(function() {{ button.textContent=old; }},1500); }}); }}); }});
